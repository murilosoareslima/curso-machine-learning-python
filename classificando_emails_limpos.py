#fazendo o mesmo que classificando_emails.py, mas limpando as stop words
...
def vetorizar_texto(texto, tradutor, stemmer):    
    ...
    for palavra in texto:
        if len(palavra) > 0:
           ...
           if raiz in tradutor:
              #pegando a posicao da palavra (raiz) no mapa
...
#colocando tudo em minusculo para evitar palavras repetidas
frases = textosPuros.str.lower();
#usando o tokenize para separar as palavras por espaços em branco e também por pontuação
textosQuebrados = [nltk.tokenize.word_tokenize(frase) for frase in frases]

#extraindo as palavras de parada em portugues
stopwords = nltk.corpus.stopwords.words("portuguese")

#set sendo utilizado no lugar de [] pois nao queremos dados repetidos no nosso dicionario, o set faz esse controle.
dicionario = set()

#funcao responsável por pegar a raiz da palavra, que por sua vez, nos permite definir quem aluno e aluna sao a mesma palavra, pos apenas o genero muda neste exemplo.
